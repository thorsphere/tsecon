...
func main() {
	...
	// 5. Create a deadline to wait for currently active requests to finish
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	// 6. Attempt to gracefully shut down the server, allowing active connections to complete
	if err := server.Shutdown(ctx); err != nil {
		tslog.Error(tserr.Op(&tserr.OpArgs{Op: "Shutdown", Fn: serverAddr, Err: err}))
	}

	// 7. Log that the server has successfully exited
